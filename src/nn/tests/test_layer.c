...
void test_create_layer() {
    ...
    vec_t *input = init_vec(1, 2, false);
    for(int i = 0; i < input->rows; i++){
        for(int j = 0; j < input->cols; j++){
            input->items[i][j] = inputs[0][j];
        }
    }


    /* copy outputs to mat structure */
    vec_t *output = init_vec(4, 1, false);
    for(int i = 0; i < output->rows; i++){
        for(int j = 0; j < output->cols; j++){
            output->items[i][j] = outputs[i][j];
        }
    }

	printf("test creating instance of layer\n");

	/* first layer */ 
	layer_t **layers = malloc(2 * sizeof(layer_t*)); 
	layer_t *input_layer = init_layer(2, 3, false); 
   	layer_t *hidden = init_layer(3, 1, false); 
...
void test_forward() {
    ...
    /* copy outputs to mat structure */
    vec_t *output = init_vec(1, 1, false);
    for(int i = 0; i < output->rows; i++){
        for(int j = 0; j < output->cols; j++){
            output->items[i][j] = outputs[0][j];
        }
    }
    ...
    layer_t *l1 = init_layer(2, 3, false); 
    layer_t *a1 = init_layer(0, 0, true);

    layer_t *l2 = init_layer(3, 1, false); 
    layer_t *a2 = init_layer(0, 0, true); 
